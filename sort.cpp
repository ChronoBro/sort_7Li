...
int main(int argc,char *argv[])
{
  ...
  //target with each run number
  ifstream targetFile;
  targetFile.open("target.run");
  ...
  for (;;)  // loop over run numbers
      ...
      for (int iExtra=0;iExtra<3;iExtra++) //loop over split evtfiles
	{
	 ...
	 Det.Silicon->Telescope[0]->target = target[(number-41)];
	 Det.Silicon->Telescope[1]->target = target[(number-41)];
	 // just to check
	 if (number!=run[(number-41)]){cout << "fix target to run no." << run[(number-41)]<< endl;}
  ...
  cout << "Total Number in S2 = " << Det.S2_count << endl;

  cout << "Total Number in RusS2 = " << Det.RusS2_count << endl;

  cout << "Checking Total = " << Det.Rus_count+Det.S2_count+Det.RusS2_count << endl;
