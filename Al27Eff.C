...
void Al27Eff()
{
  ...
  */
  ...
  for(int i=1;i<=125;i++)
    {
      ...
      //if(Be1D->GetBinCenter(i)>17.)Be1D->SetBinContent(i,0.);
    }
  ...
  TH1F * relative = (TH1F*)data->Get("/corr/Li7/Li7_theta_reactCoM_27Al")->Clone("73");
  for(int i=0;i<125;i++)
    {
      if(relative->GetBinCenter(i)>17.5)relative->SetBinContent(i,0.);
    }
